...
# 3. Create budgets at each scope level in the hierarchy.
#    The protocol server walks from the toolset up to the tenant root,
#    so every ancestor needs an allocated budget.
for SCOPE in \
  "tenant:$TENANT_ID" \
...
for TOOLSET in "internal-notes" "crm-updates"; do
  ...
  if [ "$HTTP_CODE" != "200" ] && [ "$HTTP_CODE" != "201" ] && [ "$HTTP_CODE" != "409" ]; then
    echo "" >&2
    echo "ERROR: Failed to create toolset budget (HTTP $HTTP_CODE, toolset=$TOOLSET)." >&2
    echo "  Check logs with: docker compose logs cycles-admin" >&2
    exit 1
  fi
done
